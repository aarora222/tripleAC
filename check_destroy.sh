...
if [[ "$#" -ne 4 ]]
then
  echo "usage: check_destroy.sh [name of container] [directory of mitm logs] [external IP] [type of container (1-8)]"
    exit 1
fi
...
finger_output=$(sudo lxc-attach -n "$1" -- finger admin)
...
then 
  ...
  sed -n "$3"p not_free_ip_file.txt >> free_ip_file.txt
  # delete line from not free ip
  sed -i "$3d" not_free_ip_file.txt
